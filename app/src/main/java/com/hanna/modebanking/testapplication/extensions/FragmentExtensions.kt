package com.hanna.modebanking.testapplication.extensions
...
inline fun <reified T : Any> Fragment.extraNotNull(key: String) = lazy {
    val value = arguments?.get(key)
        ?: throw IllegalStateException("expected argument with key $key was not in the argument bundle")
    value as? T
        ?: throw IllegalStateException("expected argument with key $key is not of the expected type")
}